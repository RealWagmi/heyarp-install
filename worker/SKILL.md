...
- On-chain actions (`escrow accept` / `submit-work` / `claim`) resolve the RPC from `--rpc-url` → `heyarp config get rpc.<network>`; the Solana program id auto-discovers from the server (pin with `--program-id`). **eip155-priced orders** additionally need `--network <network>` on escrow commands and the `contract.<network>` config (README §2) — plus gas on your `0x` settlement address (the eip155 stake is tiny, e.g. 0.0001 ETH on robinhood-testnet; live values: `heyarp escrow info`).
